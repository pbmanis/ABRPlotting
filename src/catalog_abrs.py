...
def get_metadata(dirname, filepath=None):
    ...
    dirname = dirname[:8] + dirname[8:].replace(
        "-", "_"
    )  # replace hyphens but only after date portion of name
    ...
    r_id = re.compile(
        "^[a-z]{1,3}[0-9]{1,3}$", re.IGNORECASE
    )  # two letters followed by 1 or 2 numbers is usually the ID
    r_exposure = re.compile(
        r"(?P<dur>[0-9]{1})?(?P<week>wk{0,1})?(?P<un>un{0,1}|noise{0,1}|sham{0,1})?(?P<exp>Exposed{1}|exposed{1}|exposure{1}|control{1})",
        re.IGNORECASE,
    )
    strains = [
        "VGAT",
    ...
    for i, fs in enumerate(fstr):
        ...
        if exp is not None:
            ...
            if exp.group("exp") is not None:
                ...
                if (
                    exp.group("wk") is not None
                    or exp.group("dur") is not None
                    or exp.group("exp") is not None
                ):
...
def highlight_by_name(row):
    colors = {
        "Ruili": "#c5d7a5",  # "darkseagreen",
        "Ruilis ABRs": "yellow",
        "Ruili ABR data": "yellow",
        "Yong's ABRs": "linen",
        "Reggie_CBA_Age": "thistle",
        "Reggie_NIHL": "plum",
        "Reggie_PreNoiseExp": "yellow",
        "Reggie_B2sMath1Cre": "lightcoral",
        "Tessa_BNE": "lavender",
        "Tessa_CBA": "turquoise",
        "Tessa_FVB": "slategrey",
...
def make_excel(df: object, outfile: Path):
    ...
    for i, column in enumerate(df):
        ...
        if column in resultno:
            ...
            writer.sheets["Sheet1"].set_column(
                first_col=i + 1, last_col=i + 1, width=column_width, cell_format=wrapit
            )  # column_dimensions[str(column.title())].width = column_width
...
def make_excel_catalog():
    ...
    for f in abrf:
        ...
        if (
            f.suffix.strip()
            in [".pdf", ".pzf", ".pzfx", ".txt", ".xls", ".xlsx", ".csv", ".pxp", ".cal", ".m"]
            or f.name == ".DS_Store"
        ):
            continue
        if f.is_dir():
            ...
            runs = np.nan
        mdata = get_metadata(
            dirname=subdir, filepath=f
        )  # get some metadata from the subdir name: multiple parses...
        abr_f.append(
            {
                "Dataset": fp[5],
                "Date": mdata.Date,
                ...
                "Strain": mdata.Strain,
                "Sex": mdata.Sex,
                "Animal_identifier": mdata.Animal_identifier,
                "Treatment": mdata.Treatment,
                "Genotype": mdata.Genotype,
                "Cross": mdata.Cross,
                "DataDirectory": subdir,
                "Runs": runs,
                "BasePath": str(Path(*fp[:5])),
            }
        )
